...
macro_rules! feed_source {
    ...
    ($name:ident, $feed_url_template:expr, $profile_url_template:expr, $auth:expr, $caps:expr, $quirks:expr) => {
        #[derive(Debug, Clone)]
        pub struct $name {
        ...
        $crate::sources::feed::feed_social_source!($name, $auth, $caps, $quirks);
    };
}
...
pub(crate) use fixed::fixed_feed_source;
